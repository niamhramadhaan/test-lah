...
export function GenerateTestModal({ open, onClose, node, onGenerate }: GenerateTestModalProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const cases = await generateTestCases(title, prompt, activeProvider!.apiKey, activeProviderId ?? undefined, activeProvider!.defaultModel, language, activeProvider!.baseUrl || '')
      onGenerate(cases)
      onClose()
    } catch (err: unknown) {
